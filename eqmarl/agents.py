...
class eQMARL_AC(MultiAgent):
    ...
    def update(self, 
        batched_states: tf.Tensor, # (n_time_steps, n_agents, ...)
        batched_joint_actions: tf.Tensor,
        batched_joint_action_probs: tf.Tensor,
        batched_next_states: tf.Tensor,
        batched_rewards: tf.Tensor, # (n_time_steps, n_agents,)
        ...
        with tf.GradientTape() as tape_actor, tf.GradientTape() as tape_critic:
            ...
            # pi({a0, a1, ...} | {s0, s1, ...})
            joint_action_probs = self.model_actor(batched_states) # (n_time_steps,n_joint_actions,)
            
            # V({s0, s1, ...})
            joint_state_values = self.model_critic(batched_states)
            
            # Actor loss.
            advantage = returns - joint_state_values
            advantage = tf.reduce_sum(advantage, axis=1) # Sum over all agent advantages.
            id_action_pairs = np.array([(i,a) for i, a in enumerate(batched_joint_actions)]) # (n_time_steps, 2,) where 2 is tuple(index, joint_action)
            probs_of_chosen_actions = tf.gather_nd(joint_action_probs, id_action_pairs) # (n_time_steps,)
            action_probs_log = tf.math.log(probs_of_chosen_actions) # (n_time_steps,)
            actor_loss = -tf.math.reduce_sum(action_probs_log * advantage)
            
            # Critic loss.
            critic_loss = huber_loss(joint_state_values, returns)
