...
def single_encoding_layered_variational_circuit(
    qubits: list,
    d_qubits: int, # Number of qubits per partition.
    n_layers: int,
    decompose: bool = False,
    ...
    """Trainable circuit with a single encoding layer at the beginning followed by cascaded variational layers."""
    shape_var = variational_layer_cls.get_shape(d_qubits)
    shape_enc = encoding_layer_cls.get_shape(d_qubits)
    ...
    #
    ## Variational shape
    theta_var = sympy.symbols(f'var(0:{n_layers})_' + '_'.join(f'(0:{x})' for x in shape_var))
    theta_var = np.asarray(theta_var).reshape((n_layers, *shape_var))
    ## Encoding shape
    theta_enc = sympy.symbols(f'enc_' + '_'.join(f'(0:{x})' for x in shape_enc))
    theta_enc = np.asarray(theta_enc).reshape(shape_enc)
    ...
    ops.append(
        encoding_layer_cls(theta_enc, name=f'e')(*qubits)
    )
...
def partite_single_encoding_layered_variational_circuit(
    qubits: list,
    n_parts: int, # Number of partitions.
    d_qubits: int, # Number of qubits per partition.
    n_layers: int,
    ...
    """eQMARL variant of the `single_encoding_layered_variational_circuit`."""
    shape_var = variational_layer_cls.get_shape(d_qubits)
    shape_enc = encoding_layer_cls.get_shape(d_qubits)
    ...
    #
    ## Variational shape
    theta_var = sympy.symbols(f'var^{{(0:{n_parts})}}(0:{n_layers})_' + '_'.join(f'(0:{x})' for x in shape_var))
    theta_var = np.asarray(theta_var).reshape((n_parts, n_layers, *shape_var))
    ## Encoding shape
    theta_enc = sympy.symbols(f'enc^{{(0:{n_parts})}}_' + '_'.join(f'(0:{x})' for x in shape_enc))
    theta_enc = np.asarray(theta_enc).reshape((n_parts, *shape_enc))
    ...
    for pidx in range(n_parts):
        ...
        # Encoding layer.
        ops.append(
            encoding_layer_cls(theta_enc[pidx], name=f'{pidx}-e')(*qubits[qidx:qidx + d_qubits])
        )
