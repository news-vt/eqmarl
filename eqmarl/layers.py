...
class HybridSingleEncodingLayeredVariationalPQC(keras.layers.Layer):
    """Trainable TensorFlow layer for quantum circuit `single_encoding_layered_variational_circuit`.
    """
    ...
    def call(self, inputs):
        ...
        #   f = feature
        # angles_enc = tf.einsum("lqf,bq->blqf", self.w_enc, inputs)
        angles_enc = tf.einsum("qf,bqf->bqf", self.w_enc, inputs) # For each partition `p`, encode each `input` state feature `q` on the `q-th` qubit and repeat encoding on same qubit for every layer `l`. Number of input features must match number of qubits.
        # _,p,q,f = inputs.shape
        # angles_enc = tf.reshape(tf.tile(inputs, multiples=[1, 1, self.n_layers, 1]), shape=(-1,p,self.n_layers,q,f))
    ...
    @staticmethod
    def generate_circuit(*args, **kwargs):
        return single_encoding_layered_variational_circuit(*args, **kwargs)


class HybridPartiteSingleEncodingLayeredVariationalPQC(keras.layers.Layer):
    """Trainable TensorFlow layer for quantum circuit `partite_single_encoding_layered_variational_circuit`.
    """
    ...
    def call(self, inputs):
        ...
        #   f = feature
        # angles_enc = tf.einsum("lqf,bq->blqf", self.w_enc, inputs)
        angles_enc = tf.einsum("pqf,bpqf->bpqf", self.w_enc, inputs) # For each partition `p`, encode each `input` state feature `q` on the `q-th` qubit and repeat encoding on same qubit for every layer `l`. Number of input features must match number of qubits.
        # _,p,q,f = inputs.shape
        # angles_enc = tf.reshape(tf.tile(inputs, multiples=[1, 1, self.n_layers, 1]), shape=(-1,p,self.n_layers,q,f))
    ...
    @staticmethod
    def generate_circuit(*args, **kwargs):
        return partite_single_encoding_layered_variational_circuit(*args, **kwargs)