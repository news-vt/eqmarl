...
def generate_model_CoinGame4_critic_quantum_central_mdp(
    n_agents: int,
    n_layers: int,
    squash_activation: str = 'arctan', # linear, arctan/atan, tanh
    beta: float = 1.0,
    ...
    model = keras.Sequential([
            ...
            keras.layers.LocallyConnected1D(d_qubits * 3, kernel_size=1, activation=nn_activation), # Reduce last feature dimension to (n_agents,3,3), use local dense units only to separate agents.
            # keras.layers.Flatten(), # Convert to batched 1D.
            # keras.layers.Dense(n_agents * d_qubits * 3, activation=nn_activation), # Reduce last feature dimension to (d_qubits,3), where `3` is the number of rotations per qubit.
            keras.layers.Reshape((n_agents * d_qubits,3)),
            qlayer,
            keras.Sequential([
                RescaleWeighted(len(observables)),
...
def generate_model_CoinGame4_critic_quantum_nnreduce_central_pomdp(
    d_qubits: int,
    keepdims: list[int],
    n_agents: int,
    n_layers: int,
